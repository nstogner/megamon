...
func ReconcileEvents(ctx context.Context, now time.Time, ups map[string]Upness, events map[string]EventRecords, unknownThreshold float64) bool {
	...
	for key, up := range ups {
		rec := events[key]
		reconcileLog.Info("ReconcileEvents", "key", key, "tpu_accelerator", up.Attrs.TPUAccelerator, "expected", up.ExpectedCount, "ready", up.ReadyCount, "unknownCount", up.UnknownCount, "unknownThreshold", unknownThreshold)
